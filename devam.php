...
if($_POST){
		...
		if($islem){
			...
			<script type="text/javascript">swal("Giriş Başarılı!", "Başarıyla giriş yaptınız!", "success");
			</script>
			</div>			
			<?php
		} else{
			...
			<div>
			<script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
			<script type="text/javascript">swal("Giriş Hatalı!", "Bilgileriniz hatalı! Tekrar deneyiniz.", "error");
			</script>
			</div>
			<?php
		} 
...
<?php
			...
			$duyuru_oku = $duyuru_cek->fetch(PDO::FETCH_ASSOC);
			...
			<?php } else {
			echo "<h3>Bu ID ile bir duyuru mevcut değil!</h3>";
		} ?>
			<br>
